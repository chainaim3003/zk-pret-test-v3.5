...
import { loadContractPortfolio, getDefaultContractPortfolio } from './ACTUSOptimMerkleAPI.js';
 ...
 */
export async function fetchRiskLiquidityAdvancedOptimMerkleData(actusUrl, contractPortfolio) {
    try {
 ...
 */
export function processAdvancedRiskData(actusResponse, liquidityThreshold, newInvoiceAmount = 0, // Simplified - not used in calculations
newInvoiceEvaluationMonth = 1, // Simplified - not used in calculations
 ...
 */
export function buildAdvancedRiskMerkleStructure(complianceData) {
    ...
    function encodeArrayToFieldForMerkleZKCompliant(numbers) {
 ...
 */
export function calculateAdvancedRiskMetrics(complianceData, dynamicThresholds, masterConfig) {
    ...
    const periodicLiquidityRatios = [];
    const periodByPeriodCompliance = [];
    ...
    const averageLiquidityRatio = Math.round(periodicLiquidityRatios.reduce((sum, ratio) => sum + ratio, 0) / periodicLiquidityRatios.length);
 ...
 */
export function validateAdvancedRiskData(complianceData, masterConfig) {
    ...
    if (masterConfig) {
        ...
        const maxCashValue = Math.max(...complianceData.cashInflow.map(v => Math.abs(v)), ...complianceData.cashOutflow.map(v => Math.abs(v)));
 ...
 */
export function generateAdvancedRiskSummary(complianceData, riskMetrics) {
 ...
 */
export function loadAdvancedMasterConfiguration() {
    try {
 ...
 */
export function loadExecutionSettings() {
    try {
 ...
 */
export function applyDynamicThresholdStrategy(executionMode, baseThreshold, masterConfig) {
 ...
 */
export function validateFieldArithmeticConstraints(thresholds, constraints) {
...
//# sourceMappingURL=RiskLiquidityAdvancedOptimMerkleUtils.js.map