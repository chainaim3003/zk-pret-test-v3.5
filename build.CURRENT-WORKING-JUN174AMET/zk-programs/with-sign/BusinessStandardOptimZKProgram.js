...
function verifyAllPatternsWithZKRegex(blFields) {
    Provable.asProver(() => {
...
function verifyAllEnums(blFields) {
    Provable.asProver(() => {
...
function verifyTypesAndArrays(blFields) {
    Provable.asProver(() => {
...
function verifyAllFieldsComprehensively(blFields) {
    Provable.asProver(() => {
...
export const BusinessStandardOptimZKProgram = ZkProgram({
    ...
    methods: {
        proveCompliance: {
            ...
            async method(BusinessStandardDataIntegrityToProve, // publicInput
            publicInputString, // privateInputs[0]
            businessStandardOptimData, // privateInputs[1]
            comprehensiveFields, // privateInputs[2] - ALL fields from data.json
            oracleSignature // privateInputs[3]
            ) {
                // Oracle signature verification
                const complianceDataHash = Poseidon.hash(BusinessStandardOptimComplianceData.toFields(businessStandardOptimData));
                ...
                return new BusinessStandardOptimPublicOutput({
                    businessStandardDataIntegrityEvaluationId: businessStandardOptimData.businessStandardDataIntegrityEvaluationId,
...
//# sourceMappingURL=BusinessStandardOptimZKProgram.js.map