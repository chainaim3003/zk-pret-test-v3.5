 ...
 */
export async function fetchGLEIFCompanyDataWithFullDetails(companyName) {
    let BASEURL;
    let url;
...
export async function isCompanyGLEIFCompliant(companyName) {
 ...
 */
export async function fetchGLEIFDataWithFullLogging(companyName) {
    return await fetchGLEIFCompanyDataWithFullDetails(companyName);
}
export function extractGLEIFSummary(apiResponse) {
    let firstRecord;
...
export function analyzeGLEIFCompliance(apiResponse, typeOfNet) {
    const summary = extractGLEIFSummary(apiResponse);
    const issues = [];
    // Check compliance criteria
    if (!summary.legalName)
        issues.push('Missing legal name');
    if (!summary.lei)
        issues.push('Missing LEI');
    if (summary.entityStatus !== 'ACTIVE')
        issues.push('Entity status is not ACTIVE');
