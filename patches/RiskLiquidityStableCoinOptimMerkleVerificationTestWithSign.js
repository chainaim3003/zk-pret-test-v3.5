 ...
 * ====================================================================
 */
import { Mina, PrivateKey, AccountUpdate, Signature, UInt64 } from 'o1js';
import { getPrivateKeyFor } from '../../core/OracleRegistry.js';
import { fetchRiskLiquidityStableCoinOptimMerkleData, processStableCoinRiskData, buildStableCoinRiskMerkleStructure, calculateStableCoinRiskMetrics, validateStableCoinRiskData, generateStableCoinRiskSummary } from '../../utils/RiskLiquidityStableCoinOptimMerkleUtils.js';
import { loadContractPortfolio } from '../../utils/ACTUSOptimMerkleAPI.js';
import { RiskLiquidityStableCoinOptimMerkleZKProgramWithSign, createStableCoinRiskComplianceData, validateStableCoinRiskComplianceData } from '../../zk-programs/with-sign/RiskLiquidityStableCoinOptimMerkleZKProgramWithSign.js';
...
export async function executeRiskLiquidityStableCoinOptimMerkleVerification(backingRatioThreshold = 100, liquidityRatioThreshold = 20, concentrationLimit = 25, qualityThreshold = 80, actusUrl = 'http://localhost:8083/eventsBatch', contractPortfolio, regulatoryFramework, jurisdictionOverride // NEW: CLI jurisdiction parameter
) {
    ...
    try {
        ...
        const stableCoinRiskData = await processStableCoinRiskData(actusResponse, contracts, // Pass contracts for principal-based analysis
        backingRatioThreshold, liquidityRatioThreshold, concentrationLimit, qualityThreshold, 1000000, // outstandingTokensAmount (will be overridden by actual liability amounts)
        1.0, // tokenValue
        ...
        console.log('📋 Creating ZK compliance data structure...');
        const zkComplianceData = createStableCoinRiskComplianceData(stableCoinRiskData.companyID, stableCoinRiskData.companyName, reserveComponents, tokenInfo, qualityMetrics, thresholds, additionalParams, merkleRoot, calculatedMetrics, 
        ...
        const proof = await RiskLiquidityStableCoinOptimMerkleZKProgramWithSign.proveStableCoinRiskCompliance(currentTimestamp, zkComplianceData, oracleSignature, merkleStructure.witnesses.companyInfo, merkleStructure.witnesses.reserves, merkleStructure.witnesses.tokens, merkleStructure.witnesses.qualityMetrics, merkleStructure.witnesses.thresholds);
...
async function loadJurisdictionThresholds(jurisdiction) {
    ...
    try {
        ...
        const settingsPath = path.join(__dirname, '../../../src/data/RISK/StableCoin/SETTINGS', `${jurisdiction}-Professional-Thresholds.json`);
...
async function main() {
    ...
    const result = await executeRiskLiquidityStableCoinOptimMerkleVerification(backingRatioThreshold, liquidityRatioThreshold, finalConcentrationLimit, qualityThreshold, actusUrl, finalContractPortfolio, undefined, // No longer using regulatoryFramework from config
...
//# sourceMappingURL=RiskLiquidityStableCoinOptimMerkleVerificationTestWithSign.js.map