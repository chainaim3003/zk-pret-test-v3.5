...
import { GenericRiskProcessor, GenericTemporalRiskZKProgram, convertToGenericFormat } from './GenericTemporalRiskFramework.js';
...
export class Basel3Processor extends GenericRiskProcessor {
    constructor() {
        super(...arguments);
        this.periodType = 'period-agnostic'; // Works with any time unit
    }
    /**
     ...
     */
    classifyHQLAForPeriod(period, temporalData) {
        let hqla = {
            L1: 0,
            L2A: 0,
            L2B: 0,
...
// =================================== Basel3 ZK Program ===================================
export class Basel3ZKProgram extends GenericTemporalRiskZKProgram {
    constructor() {
        super(...arguments);
        this.periodType = 'period-agnostic'; // Works with any time unit
    }
     ...
     */
    riskCalculation(data, thresholds) {
 ...
 */
export function createCompatibleACTUSData(genericData, scenarioParams = {}) {
    ...
    const compatibleData = new ACTUSDatao1({
        ...
        cashInflows: [cashInflows].flat(),
        cashOutflows: [cashOutflows].flat(),
 ...
 */
export function convertACTUSToGenericTemporal(inflows, outflows, monthsCount, rawEvents, contractDetails) {
    ...
    const enhancedRawEvents = rawEvents.map((eventContract, index) => {
        ...
        const matchingContract = contractsWithHQLA.find(contract => contract.contractID === eventContract.contractId);
    ...
    const temporalData = convertToGenericFormat(monthsCount, 'monthly', inflows, outflows, enhancedRawEvents, // ✅ Use enhanced events with HQLA classifications
 ...
 */
export async function processBasel3ThroughGenericFramework(inflows, outflows, monthsCount, rawEvents, contractDetails, thresholds) {
    console.log('🚀 Processing Basel3 data through generic framework...');
    // Step 1: Convert to generic temporal format
    const temporalData = convertACTUSToGenericTemporal(inflows, outflows, monthsCount, rawEvents, contractDetails);
...
//# sourceMappingURL=Basel3Implementation.js.map