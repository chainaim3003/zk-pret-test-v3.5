 ...
 */
export async function processBasel3RiskDataOptimMerkle(actusResponse, lcrThreshold, nsfrThreshold = 100, liquidityThreshold = 10, newInvoiceAmount = 5000, newInvoiceEvaluationMonth = 11) {
    ...
    const nsfrComponents = calculateBasel3CompliantNSFR(actusResponse.contractDetails, actusResponse.rawResponseData, actusResponse.periodsCount);
    return {
 ...
 */
function categorizeContractsForHQLAOptimMerkle(contractDetails, inflowData) {
 ...
 */
function calculateBasel3CompliantNSFR(contractDetails, rawActusData, periodsCount) {
    ...
    contractDetails.forEach((contract, index) => {
        ...
        const rawContract = Array.isArray(rawActusData) ? rawActusData.find((raw) => raw.contractId === contractID) : {};
        ...
        if (rawContract && rawContract.events && rawContract.events.length > 0) {
            ...
            const iedEvent = rawContract.events.find((event) => event.type === 'IED');
            if (iedEvent) {
        ...
        const hasNegativeIED = rawContract?.events?.some((event) => event.type === 'IED' && event.payoff < 0);
 ...
 */
function calculateNSFRComponentsOptimMerkle(hqlaCategories, inflows, outflows) {
 ...
 */
function calculateStressedOutflowsOptimMerkle(baseOutflows) {
 ...
 */
function calculateAndDisplayMonthlyDetails(complianceData, lcrThreshold) {
 ...
 */
export function calculateBasel3RiskMetricsOptimMerkle(complianceData) {
    ...
    let cumulativeASF = 0;
    let cumulativeRSF = 0;
    const cumulativeLCRRatios = [];
    const cumulativeNSFRRatios = [];
 ...
 */
export function validateBasel3RiskDataOptimMerkle(complianceData) {
 ...
 */
export function generateBasel3RiskSummaryOptimMerkle(complianceData, riskMetrics) {
    return `
...
//# sourceMappingURL=RiskLiquidityBasel3OptimMerkleUtils.js.map