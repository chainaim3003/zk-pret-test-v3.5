 ...
 */
export async function getRegulatoryFrameworks(jurisdiction) {
 ...
 */
export async function getBackingRatioRequirement(jurisdiction, framework) {
 ...
 */
function getDefaultRegulatoryConfig() {
    return {
        US: {
            frameworks: {
                STABLE: {
 ...
 */
export async function validateRegulatoryCompliance(assetContracts, jurisdiction) {
    ...
    for (const [frameworkName, frameworkConfig] of Object.entries(frameworks)) {
        ...
        const weight = frameworkConfig.weight || 1.0;
 ...
 */
export async function validateFrameworkCompliance(assetContracts, frameworkName, frameworkConfig) {
    const violations = [];
...
export class RegulatoryConfigManager {
     ...
     */
    static async demonstrateMiCABackingChange() {
     ...
     */
    static async getCurrentBackingRequirements() {
        ...
        for (const jurisdiction of ['US', 'EU']) {
            ...
            for (const [frameworkName, frameworkConfig] of Object.entries(frameworks)) {
                const backingRatio = frameworkConfig.minimumBackingRatio || 100;
                const weight = frameworkConfig.weight || 1.0;
...
//# sourceMappingURL=ConfigurableRegulatoryFrameworks.js.map