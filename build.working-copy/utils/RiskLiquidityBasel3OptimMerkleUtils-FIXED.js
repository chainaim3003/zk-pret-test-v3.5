...
import { buildMerkleTreeZK, MerkleWitness8 } from './CoreZKUtilities.js';
// =================================== Data Processing Functions ===================================
/**
 * Fetch and process ACTUS data for Basel3 Risk scenario
 */
export async function fetchRiskLiquidityBasel3OptimMerkleData(actusUrl, contractPortfolio) {
    try {
 ...
 */
export async function processBasel3RiskData(actusResponse, lcrThreshold, nsfrThreshold = 100, liquidityThreshold = 10, newInvoiceAmount = 5000, newInvoiceEvaluationMonth = 11) {
    ...
    try {
        ...
        const rawEventsData = actusResponse.rawResponseData || [];
        ...
        const enhancedResults = await processBasel3ThroughGenericFramework(actusResponse.inflow, actusResponse.outflow, actusResponse.periodsCount, rawEventsData, // ✅ FIX: Use actual ACTUS response data with events
        ...
        return {
            ...
            riskEvaluated: 1,
            cashInflow: actusResponse.inflow.map((period) => period.reduce((sum, value) => sum + value, 0)),
            cashOutflow: actusResponse.outflow.map((period) => period.reduce((sum, value) => sum + value, 0)),
 ...
 * Extract HQLA data from generic framework results
 */
function extractHQLAFromGenericResults(enhancedResults) {
 ...
 */
function processBasel3RiskDataLegacy(actusResponse, lcrThreshold, nsfrThreshold = 100, liquidityThreshold = 10, newInvoiceAmount = 5000, newInvoiceEvaluationMonth = 11) {
 ...
 */
function categorizeContractsForHQLA(contractDetails, inflowData) {
 ...
 */
function calculateStressedOutflows(baseOutflows) {
    // Basel3 stress factors
    const STRESS_FACTORS = {
        retailDeposits: 0.03,
        wholesaleDeposits: 0.25,
        securedFunding: 1.0,
 ...
 */
export function buildBasel3RiskMerkleStructure(complianceData) {
 ...
 */
export function calculateBasel3RiskMetrics(complianceData) {
 ...
 */
export function validateBasel3RiskData(complianceData) {
 ...
 */
export function generateBasel3RiskSummary(complianceData, riskMetrics) {
    return `
...
//# sourceMappingURL=RiskLiquidityBasel3OptimMerkleUtils-FIXED.js.map