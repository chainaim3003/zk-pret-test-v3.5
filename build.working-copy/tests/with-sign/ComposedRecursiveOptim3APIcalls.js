 ...
 */
export async function fetchOptimCorporateRegistrationData(companyIdentifier) {
 ...
 */
export async function fetchOptimEXIMData(companyName) {
 ...
 * Supports company name queries
 */
export async function fetchOptimGLEIFData(companyName) {
 ...
 */
export function convertToCorporateRegistrationOptimData(apiResponse, companyIdentifier) {
    ...
    return new CorporateRegistrationOptimComplianceData({
        ...
        companyStatus: CircuitString.fromString(masterData['company_status(for_efiling)'] || masterData.company_status || 'ACTIVE'),
        ...
        numberOfPartners: CircuitString.fromString(masterData.number_of_partners ? String(masterData.number_of_partners) : '0'),
 ...
 */
export function convertToEXIMOptimData(apiResponse, companyName) {
 ...
 */
export function convertToGLEIFOptimData(apiResponse, companyName) {
    ...
    return new GLEIFOptimComplianceData({
        ...
        name: CircuitString.fromString(firstRecord?.attributes?.entity?.legalName?.name || companyName),
        entity_status: CircuitString.fromString('ACTIVE'),
        registration_status: CircuitString.fromString(firstRecord?.attributes?.registration?.status || 'ISSUED'),
 ...
 */
function isCINFormat(identifier) {
 ...
 */
export function calculateComplianceScore(corpRegData, eximData, gleifData) {
    // Corporate Registration compliance scoring
    let corpRegScore = 0;
    const masterData = corpRegData.data?.company_master_data;
    if (masterData) {
        if (masterData.company_status === 'ACTIVE' || masterData['company_status(for_efiling)'] === 'ACTIVE')
            corpRegScore += 40;
        if (masterData.cin)
            corpRegScore += 30;
        if (masterData.date_of_incorporation)
            corpRegScore += 15;
        if (masterData.company_category)
            corpRegScore += 15;
    }
    ...
    if (eximData.iec)
        eximScore += 50;
    if (eximData.iecStatus === 1)
        eximScore += 50;
    // GLEIF compliance scoring
    let gleifScore = 0;
    const gleifRecord = gleifData.data?.[0];
    if (gleifRecord) {
        if (gleifRecord.attributes?.lei)
            gleifScore += 50;
        if (gleifRecord.attributes?.registration?.status === 'ISSUED')
            gleifScore += 50;
    }
...
function generateMockCorporateRegistrationData(identifier) {
    return {
        data: {
            company_master_data: {
...
function generateMockEXIMData(companyName) {
    return {
...
function generateMockGLEIFData(companyName) {
    return {
        data: [{
                type: 'lei-records',
 ...
 */
export async function fetchAllServiceDataForCompanies(companyNames, typeOfNet) {
...
//# sourceMappingURL=ComposedRecursiveOptim3APIcalls.js.map