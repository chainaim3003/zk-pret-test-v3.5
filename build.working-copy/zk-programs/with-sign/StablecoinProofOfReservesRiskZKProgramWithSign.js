import { Field, Signature, Struct, ZkProgram, CircuitString, Bool, Poseidon } from 'o1js';
...
export const LiquidityRatioZkprogram = ZkProgram({
    ...
    methods: {
        proveCompliance: {
            ...
            async method(LiquidityRatioToProve, LiquidityRatioData, oracleSignature) {
                let out = Bool(true);
                //try {
                //========================================================================================================
                const risk1DataHash = Poseidon.hash(ACTUSData.toFields(LiquidityRatioData));
                const registryPublicKey = getPublicKeyFor('RISK');
                const isValidSignature = oracleSignature.verify(registryPublicKey, [risk1DataHash]);
                ...
                for (let month = 0; month < monthsCount.toBigInt(); month++) {
                    ...
                    if (LCR < liquidityThreshold_LCR.mul(100)) {
                        out = Bool(false);
                    }
                    // Store results
                    monthlyLCR.push({
...
//# sourceMappingURL=StablecoinProofOfReservesRiskZKProgramWithSign.js.map