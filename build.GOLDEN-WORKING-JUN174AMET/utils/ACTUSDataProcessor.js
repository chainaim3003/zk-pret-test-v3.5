 ...
 */
export function printCoreACTUSResponse(rawResponse, apiUrl) {
    ...
    if (data) {
        ...
        if (data.inflow)
            console.log(`Inflow periods: ${Array.isArray(data.inflow) ? data.inflow.length : 'Not array'}`);
        if (data.outflow)
            console.log(`Outflow periods: ${Array.isArray(data.outflow) ? data.outflow.length : 'Not array'}`);
        if (data.monthsCount !== undefined)
            console.log(`Months count: ${data.monthsCount}`);
        if (data.periodsCount !== undefined)
            console.log(`Periods count: ${data.periodsCount}`);
        if (data.contractDetails)
            console.log(`Contract details: ${Array.isArray(data.contractDetails) ? data.contractDetails.length : 'Not array'} contracts`);
    }
 ...
 */
export function processRawACTUSData(rawData) {
    ...
    const allDates = parsedData.flatMap(contract => contract.events?.map(event => new Date(event.time)) || []);
...
function classifyContract(contract) {
    if (contract.contractId === "pam01")
        return "L2A"; // Long-term PAM = Level 2A (15% haircut) - MATCH OLD TEST
    if (contract.contractId === "pam02")
        return "L1"; // Cash = Level 1 (0% haircut) - MATCH OLD TEST
 ...
 */
function getHQLAbyMonth(classifiedContracts, minDate) {
    let hqlamap = {};
 ...
 */
export function convertToOptimMerkleFormat(processedData) {
 ...
 */
export async function callACTUSAPIWithPostProcessing(url, contracts, riskFactors = []) {
    ...
    try {
        ...
        const transformedData = rawData.map((contractResponse) => ({
...
//# sourceMappingURL=ACTUSDataProcessor.js.map