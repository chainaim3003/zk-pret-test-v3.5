...
dotenv.config();
import { Field, Mina, PrivateKey, AccountUpdate, CircuitString, Poseidon, UInt64, MerkleTree } from 'o1js';
import { ComposedOptimCompliance } from './ComposedRecursiveOptim3LevelZKProgramWithSign.js';
...
class ComposedProofRegistry {
     ...
     */
    getLatestProof(companyId) {
        const companyProofs = this.proofs.get(companyId);
        if (!companyProofs || companyProofs.length === 0)
            return null;
     ...
     */
    getProofLineageExamples(maxExamples = 3) {
        const examples = [];
        let count = 0;
...
export async function getComposedRecursiveOptim3LevelVerificationWithSignUtils(companyName, companyCIN, testIterations = 1) {
    ...
    try {
        ...
        for (let iteration = 1; iteration <= testIterations; iteration++) {
            ...
            try {
                ...
                const corpRegResult = await getCorporateRegistrationOptimSingleCompanyVerificationWithSignUtils(companyCIN // Use CIN for Corporate Registration
                );
                ...
                const eximResult = await getEXIMOptimSingleCompanyVerificationWithSignUtils(companyName // Use Company Name for EXIM
                );
                ...
                const gleifResult = await getGLEIFOptimSingleCompanyVerificationWithSignUtils(companyName // Use Company Name for GLEIF
                );
                ...
                const level1Proof = await ComposedOptimCompliance.level1(currentTimestamp.value, corpRegProof);
                // Level 2: Level1 + EXIM
                console.log(`🔸 Level 2: Composing Level1 + EXIM proof...`);
                const level2Proof = await ComposedOptimCompliance.level2(currentTimestamp.value, level1Proof, eximProof);
                // Level 3: Level2 + GLEIF (Final composed proof)
                console.log(`🔸 Level 3: Composing Level2 + GLEIF proof (Final)...`);
                const finalComposedProof = await ComposedOptimCompliance.level3(currentTimestamp.value, level2Proof, gleifProof);
                ...
                const verifyTxn = await Mina.transaction(senderAccount, async () => {
                    await zkApp.verifyAndStoreComposedProof(finalComposedProof, CircuitString.fromString(companyIdentifier), witness);
                });
        ...
        if (companyResult.iterations.length > 1) {
            ...
            if (scores.length >= 2) {
                ...
                if (scoreDiff > 5)
                    companyResult.complianceTrend = 'improving';
                else if (scoreDiff < -5)
                    companyResult.complianceTrend = 'declining';
                else
                    companyResult.complianceTrend = 'stable';
            }
        }
...
//# sourceMappingURL=ComposedRecursiveOptim3LevelVerificationTestWithSignUtils.js.map