...
import { BusinessStandardOptimZKProgram, BusinessStandardOptimComplianceData, ComprehensiveOptimBLFields } from '../../zk-programs/with-sign/BusinessStandardOptimZKProgram.js';
...
function extractComprehensiveOptimFields(evalBLJson) {
    ...
    const validateAndTruncate = (value, maxLength = 48) => {
        if (!value)
            return "DEFAULT";
    ...
    const getNestedValue = (obj, path, fallback = "DEFAULT") => {
        let current = obj;
    ...
    const hasNonEmptyArray = (obj, path) => {
        const value = path.reduce((current, key) => current && typeof current === 'object' ? current[key] : undefined, obj);
    ...
    const shipperPartyName = validateAndTruncate(getNestedValue(evalBLJson, ['documentParties', 'shipper', 'partyName']), 48);
    const issuingPartyName = validateAndTruncate(getNestedValue(evalBLJson, ['documentParties', 'issuingParty', 'partyName']), 48);
    const addressCity = validateAndTruncate(getNestedValue(evalBLJson, ['documentParties', 'issuingParty', 'address', 'city']), 48);
    const countryCode = validateAndTruncate(getNestedValue(evalBLJson, ['documentParties', 'issuingParty', 'address', 'countryCode']), 2);
...
function createOptimComplianceData(evalBLJsonFileName, evalBLJson) {
...
export async function getBSDIOptimVerificationWithSignUtils(evalBLJsonFileName) {
    ...
    const complianceDataHash = Poseidon.hash(BusinessStandardOptimComplianceData.toFields(complianceData));
    ...
    try {
        const proof = await BusinessStandardOptimZKProgram.proveCompliance(Field(1), // Public input
...
//# sourceMappingURL=BusinessStandardOptimVerificationTestUtils.js.map