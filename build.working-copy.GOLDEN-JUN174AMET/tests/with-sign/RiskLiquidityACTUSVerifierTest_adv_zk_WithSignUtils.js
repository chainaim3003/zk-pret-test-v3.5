...
export async function getRiskADVZKWithSign(userLiquidityThreshold) {
  ...
  Mina.setActiveInstance(localInstance);
    ...
    const deployTxn = await Mina.transaction(RiskProverdeployerAccount, async () => {
    ...
    const Inf = cashInfl.map(block => block.reduce((sum, value) => sum + value, 0));
    const Outf = cashOutfl.map(block => block.reduce((sum, value) => sum + value, 0));
    ...
    const risk1Data = new ACTUSData({
        ...
        liquidityThreshold: (riskScenario1Data.liquidityThreshold),
        inflowLength: riskScenario1Data.inflowLength,
    ...
    // Verify proof
    const txn = await Mina.transaction(RiskProversenderAccount, async () => {
...
//# sourceMappingURL=RiskLiquidityACTUSVerifierTest_adv_zk_WithSignUtils.js.map