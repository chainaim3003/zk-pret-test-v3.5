import { MerkleTree, MerkleWitness, CircuitString, Poseidon } from 'o1js';
...
export class BusinessStdMerkleTree {
    ...
    parseBLToTreeData(blData) {
        ...
        const createDataFieldEntry = (key, value, fieldName) => {
            let safeValue;
            if (typeof value === 'boolean') {
                safeValue = value ? 'true' : 'false';
            }
        ...
        const extractCountryCode = (unLocationCode) => {
            if (!unLocationCode || unLocationCode.length < 2)
                return '';
        ...
        return [
            ...
            createZKFieldEntry(0, blData.transportDocumentReference, 'transportDocumentReference'),
            createZKFieldEntry(1, shipper.partyName, 'shipperPartyName'),
            createZKFieldEntry(2, issuingParty.partyName, 'issuingPartyName'),
            createZKFieldEntry(3, issuingAddress.city, 'addressCity'),
            createZKFieldEntry(4, issuingAddress.countryCode, 'countryCode'),
            createZKFieldEntry(5, blData.carrierCode, 'carrierCode'),
            // Enum validation fields (6-9) - need witnesses for enum checks
            createZKFieldEntry(6, blData.transportDocumentTypeCode, 'transportDocumentTypeCode'),
            createZKFieldEntry(7, blData.receiptTypeAtOrigin, 'receiptTypeAtOrigin'),
            createZKFieldEntry(8, blData.deliveryTypeAtDestination, 'deliveryTypeAtDestination'),
            createZKFieldEntry(9, blData.carrierCodeListProvider, 'carrierCodeListProvider'),
            // Boolean validation fields (10-12) - need witnesses for boolean checks
            createZKFieldEntry(10, blData.isShippedOnBoardType, 'isShippedOnBoardType'),
            createZKFieldEntry(11, blData.isElectronic, 'isElectronic'),
            createZKFieldEntry(12, blData.isToOrder, 'isToOrder'),
            ...
            createZKFieldEntry(24, firstVoyage.vesselName, 'vesselName'),
            createZKFieldEntry(25, firstVoyage.carrierExportVoyageNumber, 'carrierExportVoyageNumber'),
            createZKFieldEntry(26, equipment.equipmentReference, 'equipmentReference'),
            createZKFieldEntry(27, firstConsignment.carrierBookingReference, 'carrierBookingReference'),
            createZKFieldEntry(28, blData.serviceContractReference, 'serviceContractReference'),
            createZKFieldEntry(29, firstSeal.number, 'sealNumber'),
            // Additional fun1 fields (30-32) - Two uppercase letters
            createZKFieldEntry(30, firstCharge.currencyCode, 'currencyCode'),
            createZKFieldEntry(31, extractCountryCode(portOfLoading.UNLocationCode), 'portOfLoadingCountry'),
            createZKFieldEntry(32, extractCountryCode(portOfDischarge.UNLocationCode), 'portOfDischargeCountry'),
            // Additional fun2 fields (33-37) - One or more non-whitespace
            createZKFieldEntry(33, equipment.ISOEquipmentCode, 'ISOEquipmentCode'),
            createZKFieldEntry(34, outerPackaging.packageCode, 'packageCode'),
            createZKFieldEntry(35, firstCharge.paymentTermCode, 'paymentTermCode'),
            createZKFieldEntry(36, blData.freightPaymentTermCode, 'freightPaymentTermCode'),
            createZKFieldEntry(37, portOfLoading.UNLocationCode, 'portOfLoadingCode'),
...
export class BusinessStdMerkleUtils {
    ...
    static createBusinessStdMerkleTree(blData) {
    ...
    static getFieldWitnesses(tree, fieldNames) {
        return fieldNames.map(fieldName => {
            const index = this.FIELD_INDICES[fieldName];
            if (index === undefined) {
    ...
    static getFieldValues(tree, fieldNames) {
        return fieldNames.map(fieldName => {
            const index = this.FIELD_INDICES[fieldName];
            if (index === undefined) {
                throw new Error(`Unknown field name: ${fieldName}. Available fields: ${Object.keys(this.FIELD_INDICES).join(', ')}`);
    ...
    static getCoreComplianceFields(tree) {
        const coreFields = [
    ...
    static getEnhancedComplianceFields(tree) {
    ...
    static verifyFieldInTree(tree, fieldName, expectedValue) {
        try {
            const index = this.FIELD_INDICES[fieldName];
            if (index === undefined)
                return false;
...
BusinessStdMerkleUtils.FIELD_INDICES = {
    // CORE 24 REQUIRED FIELDS (0-23)
    // Pattern validation fields (0-5)
    'transportDocumentReference': 0,
    'shipperPartyName': 1,
    'issuingPartyName': 2,
    'addressCity': 3,
    'countryCode': 4,
    ...
    'transportDocumentTypeCode': 6,
    'receiptTypeAtOrigin': 7,
    'deliveryTypeAtDestination': 8,
    'carrierCodeListProvider': 9,
    // Boolean validation fields (10-12)
    'isShippedOnBoardType': 10,
    'isElectronic': 11,
    'isToOrder': 12,
    // Array count fields (13-16)
    'partyContactDetailsCount': 13,
    'consignmentItemsCount': 14,
    'utilizedTransportEquipmentsCount': 15,
    'vesselVoyagesCount': 16,
    // String existence fields (17-23) - the "missing" 7 fields
    'transportDocumentStatus': 17,
    'street': 18,
    'termsAndConditions': 19,
    'cargoMovementTypeAtOrigin': 20,
    'cargoMovementTypeAtDestination': 21,
    'plannedArrivalDate': 22,
    'plannedDepartureDate': 23,
    // ADDITIONAL FIELDS USING EXISTING ZKRegex (24-37)
    // Additional fun0 fields (24-29)
    'vesselName': 24,
    'carrierExportVoyageNumber': 25,
    'equipmentReference': 26,
    'carrierBookingReference': 27,
    'serviceContractReference': 28,
    'sealNumber': 29,
    // Additional fun1 fields (30-32)
    'currencyCode': 30,
    'portOfLoadingCountry': 31,
    'portOfDischargeCountry': 32,
    // Additional fun2 fields (33-37)
    'ISOEquipmentCode': 33,
    'packageCode': 34,
    'paymentTermCode': 35,
    'freightPaymentTermCode': 36,
    'portOfLoadingCode': 37,
...
//# sourceMappingURL=BusinessStdIntegrityOptimMerkleUtils.js.map