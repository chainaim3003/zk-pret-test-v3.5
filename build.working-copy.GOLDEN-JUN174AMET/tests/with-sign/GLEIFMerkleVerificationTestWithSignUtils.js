...
dotenv.config();
import { Field, Mina, PrivateKey, AccountUpdate, Signature } from 'o1js';
...
import { GLEIFdeployerAccount, GLEIFdeployerKey, getPrivateKeyFor } from '../../core/OracleRegistry.js';
/**
 ...
 */
export async function getGLEIFMerkleVerificationUtils(companyName, fieldsToReveal = ['name', 'registration_status', 'lei']) {
    ...
    try {
        ...
        const proof = await GLEIFMerkleVerifier.proveSelectiveCompliance(Field(0), // GLEIFToProve
 ...
 */
export async function getGLEIFExtendedMerkleVerificationUtils(companyName, typeOfNet) {
    ...
    try {
        ...
        const proof = await GLEIFMerkleVerifier.proveExtendedCompliance(Field(0), companyTree.root, extendedFields.witnesses[0], // name
        ...
        console.log(`🔗 LEI: ${proof.publicOutput.lei.toString()}`);
        console.log(`🌍 Country: ${proof.publicOutput.legalAddress_country.toString()}`);
        console.log(`🏙️ City: ${proof.publicOutput.legalAddress_city.toString()}`);
        console.log(`⚖️ Jurisdiction: ${proof.publicOutput.jurisdiction.toString()}`);
 ...
 */
export async function getGLEIFComprehensiveMerkleVerificationUtils(companyName) {
    ...
    try {
        ...
        const overallCompliant = GLEIFBusinessLogicUtils.checkOverallCompliance(entityStatus, registrationStatus, conformityFlag, lastUpdate, nextRenewal, lei);
        ...
        const proof = await GLEIFComprehensiveVerifier.proveComprehensiveCompliance(Field(0), // GLEIFToProve
        companyTree.root, // Dataset root signed by oracle
 ...
 */
export async function getGLEIFComprehensiveWithSmartContractUtils(companyName) {
...
//# sourceMappingURL=GLEIFMerkleVerificationTestWithSignUtils.js.map