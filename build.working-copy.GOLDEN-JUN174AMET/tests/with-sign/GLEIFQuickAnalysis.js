        ...
        if (result.data && result.data.length > 0) {
            ...
            if (attributes.entity?.legalAddress) {
                ...
                if (addr.addressLines)
                    console.log(`      - Address Lines: ${JSON.stringify(addr.addressLines)}`);
                if (addr.city)
            ...
            if (attributes.entity?.headquartersAddress) {
                ...
                console.log('      Components available for bundling:');
                if (hq.addressLines)
                    console.log(`      - HQ Address Lines: ${JSON.stringify(hq.addressLines)}`);
                if (hq.city)
                    console.log(`      - HQ City: ${hq.city}`);
                if (hq.region)
                    console.log(`      - HQ Region: ${hq.region}`);
                if (hq.country)
                    console.log(`      - HQ Country: ${hq.country}`);
                if (hq.postalCode)
                    console.log(`      - HQ Postal Code: ${hq.postalCode}`);
                totalFields += 5; // Estimate 5 components
            }
            if (attributes.registration) {
                ...
                console.log('      Components available for bundling:');
                if (reg.initialRegistrationDate)
                    console.log(`      - Initial Registration: ${reg.initialRegistrationDate}`);
                if (reg.lastUpdateDate)
                    console.log(`      - Last Update: ${reg.lastUpdateDate}`);
                if (reg.nextRenewalDate)
                    console.log(`      - Next Renewal: ${reg.nextRenewalDate}`);
                if (reg.managingLou)
                    console.log(`      - Managing LOU: ${reg.managingLou}`);
                if (reg.corroborationLevel)
                    console.log(`      - Corroboration Level: ${reg.corroborationLevel}`);
                totalFields += 5; // Estimate 5 components
            }
...
// Run the analysis
quickGLEIFAnalysis();
//# sourceMappingURL=GLEIFQuickAnalysis.js.map