...
import { extractDateRangeFromEvents, determinePeriodType } from './GenericTemporalRiskFramework.js';
import { processBasel3ThroughGenericFramework, convertACTUSToGenericTemporal } from './Basel3Implementation.js';
 ...
 */
export async function callACTUSAPIWithGenericProcessing(url, contracts, options = {}) {
    ...
    const optimMerkleResponse = await callACTUSAPIWithPostProcessing(url, contracts, options.riskFactors || []);
    ...
    const temporalData = convertACTUSToGenericTemporal(optimMerkleResponse.inflow, optimMerkleResponse.outflow, optimMerkleResponse.periodsCount, contracts, // Use original contracts as raw events
    optimMerkleResponse.contractDetails);
    // Step 3: Apply risk framework-specific processing
    let genericClassification;
    let frameworkResults;
    switch (options.riskFramework) {
        case 'basel3':
            frameworkResults = await processBasel3ThroughGenericFramework(optimMerkleResponse.inflow, optimMerkleResponse.outflow, optimMerkleResponse.periodsCount, contracts, optimMerkleResponse.contractDetails, {
                liquidityThreshold_LCR: 100,
 ...
 */
export function autoDetectPeriodType(rawACTUSResponse, inflows, outflows) {
 ...
 */
export async function processACTUSWithAutoFramework(url, contracts, options = {}) {
    ...
    let recommendedPeriodType = 'monthly';
    let recommendedFramework = options.fallbackFramework || 'basel3';
    let reasoning = 'Using default settings';
    // Step 2: Auto-detect period type if requested
    if (options.autoDetectPeriod) {
        const detection = autoDetectPeriodType(contracts, basicResults.inflow, basicResults.outflow);
    ...
    const enhancedResults = await callACTUSAPIWithGenericProcessing(url, contracts, {
        ...
        riskFramework: recommendedFramework,
 ...
 */
function detectOptimalRiskFramework(contracts, basicResults) {
    ...
    const bestFramework = Object.entries(scores).reduce((a, b) => scores[a[0]] > scores[b[0]] ? a : b)[0];
...
//# sourceMappingURL=EnhancedACTUSDataProcessor.js.map