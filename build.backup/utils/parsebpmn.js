...
async function buildGraph(filePath) {
    const xml = await fs.readFile(filePath, 'utf8');
    const doc = new DOMParser().parseFromString(xml, 'text/xml');
    const namespace = 'http://www.omg.org/spec/BPMN/20100524/MODEL';
    const graph = {};
    const elements = {};
    const nodes = xpath.select(`//*[namespace-uri()='${namespace}']`, doc);
    ...
    const flows = xpath.select(`//*[local-name()='sequenceFlow']`, doc);
...
function findStartEvent(doc) {
    const startEvent = xpath.select(`//*[local-name()='startEvent']`, doc);
    return startEvent.length > 0 ? startEvent[0].getAttribute('id') || null : null;
}
// Find all end event IDs
function findEndEvents(doc) {
    const endEvents = new Set();
    const endNodes = xpath.select(`//*[local-name()='endEvent']`, doc);
...
function findValidCommonSuffix(arrays) {
    if (arrays.length === 0)
        return [];
...
function buildExpressionFromList(exprList) {
    if (exprList.length === 0)
        return '';
    return exprList.join('');
}
// Recursively build combined expression with factoring
function buildExpression(graph, current, ends, cache) {
...
export default async function parseBpmn(filePath) {
