...
import { Bool, Bytes } from 'o1js';
import { fun0, fun1, fun2 } from './circuitFile.js';
class Bytes50 extends Bytes(50) {
}
const execPromise = promisify(exec);
let dict = {};
export function isPatternObject(obj) {
    return obj && typeof obj.type === 'string';
}
async function verifySchema(requiredKeys, dict, exampleJson) {
    for (const key in requiredKeys) {
        ...
        if (!isPatternObject(value)) {
            const result = await verifySchema(value, dict, exampleJson[key]);
            if (!result)
                return false;
        }
        else {
...
async function regexVerify(str, functionName) {
    const functionMap = {
        "fun0": fun0,
        "fun1": fun1,
        "fun2": fun2
...
export async function verifyActual() {
    try {
...
export async function verifyActualFromFile(filename) {
    try {
        const data = await fs.promises.readFile('src/core/data.json', 'utf8');
        const jsonData = JSON.parse(data);
        const requiredKeys = jsonData.requiredKeys;
...
export async function verifyActualFromJSONString(jsonString) {
...
//# sourceMappingURL=verifyActual.js.map