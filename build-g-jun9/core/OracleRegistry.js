...
export function getPrivateKeyFor(key) {
    const privateKey = Registry.get(key)?.privateKey;
    if (!privateKey)
        throw new Error(`No private key found for ${key}`);
    return privateKey;
}
export function getPublicKeyFor(key) {
    const publicKey = Registry.get(key)?.publicKey;
    if (!publicKey)
        throw new Error(`No private key found for ${key}`);
    return publicKey;
}
//# sourceMappingURL=OracleRegistry.js.map