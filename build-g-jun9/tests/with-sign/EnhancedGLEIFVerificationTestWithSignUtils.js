...
import { fetchGLEIFFullStructure } from './GLEIFUtils.js';
/**
 ...
 */
export async function getEnhancedGLEIFVerificationWithSignUtils(companyName, typeOfNet, verificationMode = 'standard', secondaryCompanyName, historicalDays) {
    ...
    const deployTxn = await Mina.transaction(GLEIFdeployerAccount, async () => {
    ...
    let primaryParsedData;
    let secondaryParsedData = null;
    try {
    ...
    const primaryEnhancedData = createEnhancedComplianceDataFromBusinessRules(primaryParsedData, primaryComplianceScore, primaryRiskLevel, isGLEIFCompliant);
    let secondaryEnhancedData = null;
    if (verificationMode === 'group' && secondaryParsedData) {
        ...
        secondaryEnhancedData = createEnhancedComplianceDataFromBusinessRules(secondaryParsedData, secondaryComplianceScore, secondaryRiskLevel, secondaryCompliant);
    ...
    try {
        if (verificationMode === 'group' && secondaryEnhancedData) {
            ...
            proof = await GLEIFEnhancedZKProgram.proveMultiCompanyCompliance(Field(0), primaryEnhancedData, secondaryEnhancedData, oracleSignature, currentTimestamp, Field(80));
        }
        else if (verificationMode === 'historical') {
            console.log('📅 Generating historical compliance proof...');
            const historicalTimestamp = UInt64.from(Date.now() - (historicalDays || 365) * 24 * 60 * 60 * 1000);
            proof = await GLEIFEnhancedZKProgram.proveHistoricalCompliance(Field(0), primaryEnhancedData, oracleSignature, historicalTimestamp, currentTimestamp, Field(75));
        }
        else {
            console.log('🏛️ Generating standard compliance proof...');
            proof = await GLEIFEnhancedZKProgram.proveCompliance(Field(0), primaryEnhancedData, oracleSignature, currentTimestamp, Field(70), Field(3));
        }
    ...
    try {
        if (verificationMode === 'group' && secondaryEnhancedData) {
            ...
            txn = await Mina.transaction(GLEIFsenderAccount, async () => {
                await zkApp.verifyGLEIFComplianceWithParams(primaryEnhancedData, oracleSignature);
            });
        }
        else if (verificationMode === 'historical') {
 ...
 */
function displayEnhancedComplianceData(data, label) {
 ...
 */
function validateBusinessRules(parsedData) {
    const details = [];
 ...
 */
function createEnhancedComplianceDataFromBusinessRules(apiResponse, complianceScore, riskLevel, isCompliant) {
...
//# sourceMappingURL=EnhancedGLEIFVerificationTestWithSignUtils.js.map