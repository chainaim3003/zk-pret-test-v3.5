 ...
 */
async function compileWithTimeout(compileFunction, timeoutMs = 300000) {
...
async function runMerkleAnalysisTest(companyName, typeOfNet, testMode) {
    ...
    if (testMode === 'FAST') {
        ...
        try {
            ...
            const proof = await compileWithTimeout(async () => {
                return await GLEIFEnhancedZKProgram.proveCompliance(Field(0), enhancedData, oracleSignature, UInt64.from(Date.now()), Field(70), // complianceThreshold
...
//# sourceMappingURL=MerkleSimpleGLEIFTest.js.map