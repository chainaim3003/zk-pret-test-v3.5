...
function extractAddressFields(address, prefix) {
    ...
    if (address.addressLines)
        fields.push(`${prefix}_addressLines`);
    if (address.city)
        fields.push(`${prefix}_city`);
    if (address.region)
        fields.push(`${prefix}_region`);
    if (address.country)
        fields.push(`${prefix}_country`);
    if (address.postalCode)
        fields.push(`${prefix}_postalCode`);
    return fields;
}
function extractRegistrationFields(registration) {
    const fields = [];
    if (registration.initialRegistrationDate)
        fields.push('initialRegistrationDate');
    if (registration.lastUpdateDate)
        fields.push('lastUpdateDate');
    if (registration.nextRenewalDate)
        fields.push('nextRenewalDate');
    if (registration.managingLou)
        fields.push('managingLou');
    return fields;
}
...
// Export for import
export { fetchGLEIFCompanyDataWithFullDetails, analyzeForZKOptimization };
// Run if called directly
if (process.argv[1] === new URL(import.meta.url).pathname) {
    main();
}
//# sourceMappingURL=GLEIFCompleteJSONAnalysis.js.map