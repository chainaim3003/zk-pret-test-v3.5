...
import { CircuitString, Field, UInt64 } from 'o1js';
...
class UltimateGLEIFSearch {
     ...
     */
    async searchByCompanyName(companyName, options = {}) {
        ...
        const nameFilters = [
            ...
            'filter[entity.otherEntityNames]',
        ...
        for (const filter of nameFilters) {
            ...
            if (options.country)
                params['filter[entity.legalAddress.country]'] = options.country;
            if (options.entityStatus)
                params['filter[entity.status]'] = options.entityStatus;
            if (options.registrationStatus)
                params['filter[registration.registrationStatus]'] = options.registrationStatus;
     ...
     */
    async searchByFinancialCodes(options) {
     ...
     */
    async ultimateSearch(searchOptions) {
     ...
     */
    findBestNameMatch(results, targetName) {
        ...
        for (const result of results) {
            ...
            const resultWords = resultLower.split(/\s+/).filter((word) => word.length > 2);
     ...
     */
    generateSuggestions(options, searchReport) {
        const suggestions = [];
     ...
     */
    getSearchStats() {
        const recent = this.searchHistory.filter(h => Date.now() - h.timestamp < 3600000);
        return {
...
export async function fetchGLEIFCompanyData(companyName, typeOfNet = 'TESTNET') {
    try {
        ...
        const knownLEI = TEST_LEIS[normalizedName];
        if (knownLEI) {
 ...
 */
async function searchGLEIFInternal(options) {
    try {
        ...
        if (result.success && result.data) {
            ...
            const entities = result.data.data.map((item) => ({
                ...
                otherNames: item.attributes.entity.otherNames?.map((name) => name.name) || []
            }));
            return {
                success: true,
                method: result.method,
...
export class GLEIFBusinessRules {
     ...
     */
    static analyzeCompliance(apiResponse) {
     ...
     */
    static isRecentUpdate(lastUpdateDate) {
        try {
     ...
     */
    static hasValidAddresses(entity) {
     ...
     */
    static extractGroupStructure(apiResponse) {
...
export class GLEIFCircuitConverter {
     ...
     */
    static convertToEnhancedComplianceData(apiResponse, businessAnalysis) {
        const entity = apiResponse.data[0].attributes.entity;
        const registration = apiResponse.data[0].attributes.registration;
     ...
     */
    static extractCompanySummary(apiResponse) {
        const entity = apiResponse.data[0].attributes.entity;
        const registration = apiResponse.data[0].attributes.registration;
        return {
 ...
 */
export function isCompanyGLEIFCompliant(companyDataOrName, typeOfNet) {
 ...
 */
export async function fetchGLEIFFullStructure(companyName, typeOfNet = 'TESTNET') {
...
//# sourceMappingURL=GLEIFUtils.js.map