 ...
 */
export async function fetchGLEIFCompanyDataDetailed(companyName, typeOfNet) {
    let BASEURL;
    let url;
    if (!typeOfNet) {
        typeOfNet = 'TESTNET';
 ...
 */
export function printFullGLEIFStructure(gleifData) {
    console.log('\n' + '='.repeat(80));
 ...
 */
function analyzeObjectStructure(obj, indent = '', maxDepth = 3, currentDepth = 0) {
 ...
 */
export function analyzeGLEIFFieldsForZKOptimization(gleifData) {
    const record = gleifData.data[0];
 ...
 */
function extractFieldsFromPath(obj, paths) {
    const fields = {};
    paths.forEach(path => {
 ...
 */
export async function analyzeGLEIFCompanyData(companyName, typeOfNet = 'TESTNET') {
    try {
...
//# sourceMappingURL=GLEIFUtilsDetailed.js.map