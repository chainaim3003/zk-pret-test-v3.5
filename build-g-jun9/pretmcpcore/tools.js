...
export function registerPRETTools(server) {
    ...
    server.tool("get-GLEIF-full-structure", "Get the complete, unprocessed JSON structure from GLEIF API response. This returns the full JSON object for structure analysis and UI viewing without any data filtering or processing.", {
        companyName: z.string().describe("Company name for GLEIF search (e.g., 'SREE PALANI ANDAVAR AGROS PRIVATE LIMITED')"),
        typeOfNet: z.string().optional().describe("Network name (e.g., 'LOCAL OR TESTNET OR MAINNET')")
    }, async ({ companyName, typeOfNet }) => {
        try {
            console.log(`Fetching complete GLEIF structure for company: ${companyName} on network: ${typeOfNet ?? 'TESTNET'}`);
            const result = await fetchGLEIFFullStructure(companyName, typeOfNet ?? 'TESTNET');
            return {
                content: [{
                        type: "text",
    ...
    //server tool gleif api call
    server.tool("get-GLEIF-verification-with-sign", "get GLEIF data takes company name and type of net TESTNET,MAINNET,etc and get GLEIF compliance for different regions data and produces proof verified in MINA BlockChain in LOCAL,TESTNET,DEVNET,MAINNET", {
        companyName: z.string().describe("Company name for GLEIF search (e.g., 'SREE PALANI ANDAVAR AGROS PRIVATE LIMITED')"),
        typeOfNet: z.string().optional().describe("Network name (e.g., 'LOCAL OR TESTNET OR MAINNET')")
    }, async ({ companyName, typeOfNet }) => {
        try {
    ...
    server.tool("get-Corporate-Registration-verification-with-sign", "get Corporate-Registration data takes CIN and type of net TESTNET,MAINNET,etc and get Corporate-Registration compliance for different regions data and produces proof verified in MINA BlockChain in LOCAL,TESTNET,DEVNET,MAINNET", {
    ...
    server.tool("get-EXIM-verification-with-sign", "get EXIM data takes company name and type of net TESTNET,MAINNET,etc and get EXIM compliance for different regions data and produces proof verified in MINA BlockChain in LOCAL,TESTNET,DEVNET,MAINNET", {
    ...
    server.tool("get-Is-company-GLEIF-compliant", "get GLEIF data takes company name and type of net TESTNET,MAINNET,etc and get GLEIF compliance for different regions data and produces proof verified in MINA BlockChain in LOCAL,TESTNET,DEVNET,MAINNET", {
        companyName: z.string().describe("Company name for GLEIF search (e.g., 'SREE PALANI ANDAVAR AGROS PRIVATE LIMITED')"),
        typeOfNet: z.string().optional().describe("Network name (e.g., 'LOCAL OR TESTNET OR MAINNET')")
    }, async ({ companyName, typeOfNet }) => {
        try {
    ...
    server.tool("get-EXIM-data", "get EXIM data takes company name and type of net TESTNET,MAINNET,etc and get EXIM compliance data for different regions", {
    ...
    server.tool("get-CorporateRegistration-data", "get GLEIF data for a company name and depending on the environment it will call different apis example TESTNET vs MAINNET vs LOCAL", {
    ...
    server.tool("get-BSDI-compliance-verification", "Verify BSDI compliance for a company using BL JSON file and produce a ZK proof.", {
    ...
    server.tool("get-BPI-compliance-verification", "Verify BPI compliance for a company using BL JSON file and produce a ZK proof.", {
        ...
        //blJsonFilePath: z.string().describe("Path to the BL JSON file for evaluation(e.g., '.data/scf/actualBL1.json')"),
    }, async ({ businessProcessType, expectedPath, actualPath }) => {
        try {
    ...
    server.tool("get-RiskLiquidityACTUS-Verifier-Test_adv_zk", "Verify Risk Liquidity ACTUS compliance for a user liquidity threshold and produce a ZK proof.", {
        userLiquidityThreshold: z.number().describe("User liquidity threshold for evaluation (e.g., '8','9')"),
    }, async ({ userLiquidityThreshold }) => {
        try {
    ...
    server.tool("get-RiskLiquidityACTUS-Verifier-Test_Basel3_Withsign", "Verify Risk Liquidity ACTUS compliance for a user liquidity threshold and produce a ZK proof.", {
