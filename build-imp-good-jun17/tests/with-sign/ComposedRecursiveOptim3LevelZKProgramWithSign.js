import { Field, ZkProgram, Struct, SelfProof, Poseidon, Bool, UInt64 } from 'o1js';
...
export const ComposedOptimCompliance = ZkProgram({
    ...
    methods: {
        ...
        level1: {
            ...
            async method(timestamp, corpRegProof) {
                ...
                return new ComposedOptimCompliancePublicOutput({
                    ...
                    servicesIncluded: Field(1),
                    servicesCompliant: isCompliant.toField(),
                    verificationTimestamp: UInt64.from(0),
                    composedProofVersion: Field(1),
         ...
         */
        level2: {
            privateInputs: [(SelfProof), EXIMOptimProof],
            async method(timestamp, prevProof, eximProof) {
                // Verify both proofs
                prevProof.verify();
                eximProof.verify();
                // Extract data
         ...
         */
        level3: {
            privateInputs: [(SelfProof), GLEIFOptimProof],
            async method(timestamp, prevProof, gleifProof) {
                // Verify both proofs
                prevProof.verify();
                gleifProof.verify();
                // Extract data
...
export class ComposedOptimProofUtils {
     ...
     */
    static getServiceCompliance(servicesCompliant) {
     ...
     */
    static getServicesIncluded(servicesIncluded) {
     ...
     */
    static isCompleteComposedProof(proof) {
     ...
     */
    static getComplianceSummary(proof) {
        const output = proof.publicOutput;
        return {
...
//# sourceMappingURL=ComposedRecursiveOptim3LevelZKProgramWithSign.js.map