...
export class GLEIFMerkleTree {
    ...
    parseGLEIFToTreeData(parsedData) {
        ...
        const createFieldEntry = (key, value, fieldName) => {
        ...
        return [
            ...
            createFieldEntry(6, attributes.conformityFlag || registration.conformityFlag, 'conformity_flag'),
...
export class GLEIFMerkleUtils {
    ...
    static async createGLEIFMerkleTree(companyName) {
    ...
    static async createBatchMerkleTree(companyNames) {
        console.log(`🏢 Creating batch tree for ${companyNames.length} companies`);
        const companiesData = await Promise.all(companyNames.map(async (name, index) => {
    ...
    static getExtendedComplianceFields(tree) {
    ...
    static getComprehensiveComplianceFields(tree) {
...
GLEIFMerkleUtils.FIELD_INDICES = {
    // Core identifiers (0-9)
    'type': 0,
    'id': 1,
    'lei': 2,
...
export class GLEIFBusinessLogicUtils {
    ...
    static checkOverallCompliance(entityStatus, registrationStatus, conformityFlag, lastUpdate, nextRenewal, lei) {
...
//# sourceMappingURL=GLEIFMerkleUtils.js.map